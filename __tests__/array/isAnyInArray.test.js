...
test('isAnyInArray [1, 2, 3, [1], null, undefined], [-3, -5, 10]', () => {
    expect(jswl.arr.isAnyInArray(originalArray, testedArray)).toBe(false);
});

test('isAnyInArray [1, 2, 3, [1], null, undefined], [-3, -5, 1]', () => {
    expect(jswl.arr.isAnyInArray(originalArray, testedArray2)).toBe(true);
});

test('isAnyInArray [1, 2, 3, [1], null, undefined], [-3, -5, "1"]', () => {
    expect(jswl.arr.isAnyInArray(originalArray, testedArray3)).toBe(false);
});

test("isAnyInArray ['dog', 'snake', 'cat'], ['racconn', 'parrot', 'snake']", () => {
    expect(jswl.arr.isAnyInArray(originalStrArray, testedStrArray)).toBe(true);
});

test("isAnyInArray [1, 2, 3, [1], null, undefined], [-3, null, -5, '1']", () => {
    expect(jswl.arr.isAnyInArray(originalArray, testedArray4)).toBe(true);
});

test("isAnyInArray [1, 2, 3, [1], null, undefined], [-3, undefined, -5, '1']", () => {
    expect(jswl.arr.isAnyInArray(originalArray, testedArray5)).toBe(true);
});